...
import com.torrydo.weathervisualizer.common.error.ErrorType
...
sealed class Resource<T>(val data: T? = null, val exception: ErrorType? = null) {
    ...
    class Error<T>(exception: ErrorType?, data: T? = null) : Resource<T>(data, exception)
...
inline fun <T> Resource<T>.onError(onError: (exception: ErrorType?, data: T?) -> Unit): Resource<T> {
...
inline fun <T, R : Exception> Resource<T>.onErrorCatch(
    exception: KClass<R>,
    onCatch: (data: R) -> Unit,
    onError: (exception: ErrorType?, data: T?) -> Unit
): Resource<T> {
    if (this is Resource.Error && this.exception != null) {
        if (exception.isInstance(this.exception)) {
            val exc = try {
